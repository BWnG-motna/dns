...
void daniel::dns::Resource::MakeRData( uint8_t const * pBuf , uint32_t const & length ) noexcept
{
	...
	switch( qtype )
	{
		...
		case QType::GPOS :
			pRDat = new ( std::nothrow ) RR::RData_GPOS() ;
			break ;
		...
		case QType::EID :
			pRDat = new ( std::nothrow ) RR::RData_EID() ;
			break ;

		case QType::NIMLOC :
			pRDat = new ( std::nothrow ) RR::RData_NIMLOC() ;
			break ;
		...
		case QType::LP :
			pRDat = new ( std::nothrow ) RR::RData_LP( pDatagram ) ;
			break ;
		...
		case QType::CLA :
			pRDat = new ( std::nothrow ) RR::RData_CLA() ;
			break ;
