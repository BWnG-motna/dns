...
daniel::baseEncoding::Base32Hex::Base32Hex()
{

}


bool daniel::baseEncoding::Base32Hex::Encode( 
	...
	uint16_t       * pWbytes )
{
	...
	while( bitPos < bitMaxPos && encPos < enclen )
	{
		...
		if( false == Read5Bits( bits , pDat , datlen , bitPos ) )
		{
			return false ;
		}
	...
	while( padding && ( 0 != encPos % 8 ) && ( encPos < enclen ) )
	{
		pEncDat[ encPos++ ] = '=' ;
	}

	if( padding && ( 0 != encPos % 8 ) )
	{
		return false ;
	}
...
bool daniel::baseEncoding::Base32Hex::Read5Bits( uint8_t & res , uint8_t const * pDat , uint16_t const & datlen , uint32_t const & bitPos )
{
	if( nullptr == pDat || 1 > datlen )
	{
		return false ;
